...
/*   Updated: 2024/07/15 01:31:37 by trosinsk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_camera(char *line, t_main_rt *main_rt)
{
	...
	main_rt->cam = cam;
	add_to_garb_col(main_rt->garb_col, cam);
	ft_free(split);
	return (0);
}
...
// static void	print_struct(t_main_rt *main_rt)
// {
// 	printf("main_rt->cam->pov->pv_x : %f\n", main_rt->cam->pov->pv_x );
// 	printf("main_rt->cam->pov->pv_y: %f\n", main_rt->cam->pov->pv_y);
// 	printf("main_rt->cam->pov->pv_z: %f\n", main_rt->cam->pov->pv_z);
